(function() {
  ...
  if ((isIos || isAndroid) && !isStandalone) {
    checkAndShowPrompt(isIos ? 'ios' : 'android-manual');
  }
  ...
  function checkAndShowPrompt(type) {
    ...
    if (!lastShown || (now - lastShown) > 86400000 || type === 'android-native') { // Always show native prompt if available
  ...
  function showPrompt(type) {
    ...
    if (type === 'android-native') {
      ...
      content = `
        <div style="font-weight: 700; margin-bottom: 4px;">Install Sanvasify (iOS)</div>
